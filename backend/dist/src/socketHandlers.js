...
const { createLobbyState, joinLobby, leaveLobbyOrGame, startGame, endGame, applyGameAction, sanitizeStateForUser, } = require("./game/engine");
async function ensureLobby(kv) {
...
async function loadState(kv, roomId) {
...
async function saveState(kv, roomId, state) {
    const key = `room:${roomId}`;
    await kv.set(key, JSON.stringify(state));
}
async function listRooms(kv) {
    return (await kv.smembers("rooms")) || [];
}
function createSocketHandlers(io, kv, usersById) {
    return async function onConnection(socket) {
        ...
        socket.on("get_room", async (payload) => {
            try {
                const roomId = payload.roomId || "LOBBY";
        ...
        socket.on("game_action", async (payload, ack) => {
            try {
                const roomId = payload.roomId || "LOBBY";
                const state = await loadState(kv, roomId);
                if (!state) {
