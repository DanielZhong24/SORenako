...
function createDevDeck() {
    return shuffle([
        ...
        ...Array.from({ length: 2 }, () => "roadBuilding"),
        ...Array.from({ length: 2 }, () => "yearOfPlenty"),
...
function joinLobby(state, userId, username) {
...
function leaveLobbyOrGame(state, userId) {
    const nextState = cloneState(state);
...
function startGame(state, actingUserId) {
    if (state.phase !== "lobby") {
        return { ok: false, state, error: "game already started" };
    }
    if (state.players.length !== 4) {
...
function canPlaceRoad(state, userId, edgeId) {
    ...
    const touchOwnRoad = [edge.v1, edge.v2].some((vertexId) => {
        if (isBlockedRoadVertex(state, userId, vertexId))
            return false;
...
function gainResource(resources, resource, amount) {
    resources[resource] += amount;
}
function hasBankResources(bank, cost) {
    return Object.keys(cost).every((resource) => (bank[resource] || 0) >= (cost[resource] || 0));
}
function spendBankResources(bank, cost) {
    for (const resource of Object.keys(cost)) {
        bank[resource] -= cost[resource] || 0;
    }
}
function gainBankResources(bank, gain) {
    for (const resource of Object.keys(gain)) {
        bank[resource] += gain[resource] || 0;
    }
}
function distributeResourcesForRoll(state, roll) {
    if (roll === 7)
        return;
    for (const tile of state.board.tiles) {
        if (tile.number !== roll)
            continue;
        if (tile.id === state.robberTileId)
            continue;
...
function updateLongestRoadHolder(state) {
    const lengths = {};
...
function computeLongestRoadForPlayer(state, userId) {
    ...
    function dfs(vertexId, used) {
        ...
        for (const edgeId of options) {
            if (!edgeSet.has(edgeId) || used.has(edgeId))
                continue;
            const edge = state.board.edges[edgeId];
    ...
    for (const edgeId of playerEdges) {
        const edge = state.board.edges[edgeId];
        const a = dfs(edge.v1, new Set());
        const b = dfs(edge.v2, new Set());
        best = Math.max(best, a, b);
    }
    return best;
}
function updateLargestArmyHolder(state) {
    ...
    if (second && second.playedKnights === best.playedKnights) {
        if (state.largestArmyHolder === best.id)
            return;
        return;
    }
    state.largestArmyHolder = best.id;
}
function getVictoryPoints(state, playerId) {
...
function getPublicVictoryPoints(state, playerId) {
    const player = getPlayer(state, playerId);
    if (!player)
        return 0;
...
function getPortRateForPlayer(state, player, resource) {
    let bestRate = 4;
...
function setupPlace(state, userId, vertexId, edgeId) {
    const placedSettlement = setupPlaceSettlement(state, userId, vertexId);
    if (!placedSettlement.ok)
        return placedSettlement;
    return setupPlaceRoad(placedSettlement.state, userId, edgeId);
}
function setupPlaceSettlement(state, userId, vertexId) {
    if (state.phase !== "setup")
        return { ok: false, state, error: "not in setup phase" };
    const active = getActivePlayer(state);
    if (active.id !== userId)
        return { ok: false, state, error: "not your setup turn" };
    if (state.pendingSetupPlacement)
        return { ok: false, state, error: "finish the current setup placement first" };
    if (state.buildings[vertexId])
        return { ok: false, state, error: "vertex already occupied" };
    if (hasNeighborBuilding(state, vertexId))
        return { ok: false, state, error: "distance rule violated" };
    const next = cloneState(state);
    const acting = getPlayer(next, userId);
...
function setupPlaceRoad(state, userId, edgeId) {
    if (state.phase !== "setup")
        return { ok: false, state, error: "not in setup phase" };
    const active = getActivePlayer(state);
    if (active.id !== userId)
...
function rollDice(state, userId) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (!state.mustRoll)
        return { ok: false, state, error: "already rolled this turn" };
    const active = getActivePlayer(state);
    if (active.id !== userId)
...
function discardResources(state, userId, discard) {
    const required = state.pendingDiscards[userId] || 0;
    if (required <= 0)
        return { ok: false, state, error: "no discard required" };
    const next = cloneState(state);
    const player = getPlayer(next, userId);
    if (!player)
        return { ok: false, state, error: "player not found" };
    const discardCount = Object.keys(discard).reduce((sum, resource) => sum + (discard[resource] || 0), 0);
    if (discardCount !== required) {
        return { ok: false, state, error: `must discard exactly ${required}` };
    }
    for (const resource of Object.keys(discard)) {
...
function moveRobber(state, userId, tileId, targetPlayerId) {
    if (!state.pendingRobber)
        return { ok: false, state, error: "robber is not pending" };
    if (Object.keys(state.pendingDiscards).length > 0) {
        return { ok: false, state, error: "waiting on discards" };
    }
    const active = getActivePlayer(state);
    if (active.id !== userId)
        return { ok: false, state, error: "only active player can move robber" };
    const tile = state.board.tiles[tileId];
    if (!tile)
        return { ok: false, state, error: "invalid tile" };
    const next = cloneState(state);
    next.robberTileId = tileId;
    next.pendingRobber = false;
    const actor = getPlayer(next, userId);
...
function resourcesToArray(resources) {
    const out = [];
...
function buildRoad(state, userId, edgeId, freeBuild) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll dice first" };
...
function buildSettlement(state, userId, vertexId) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll dice first" };
    ...
    if (active.id !== userId)
        return { ok: false, state, error: "not your turn" };
    if (state.buildings[vertexId])
        return { ok: false, state, error: "vertex occupied" };
    if (hasNeighborBuilding(state, vertexId))
        return { ok: false, state, error: "distance rule violated" };
...
function buildCity(state, userId, vertexId) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll dice first" };
...
function buyDevCard(state, userId) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll dice first" };
    ...
    if (state.devDeck.length === 0)
        return { ok: false, state, error: "dev deck is empty" };
    const next = cloneState(state);
    const player = getPlayer(next, userId);
...
function playDevCard(state, userId, card, payload) {
    ...
    if (state.playedDevCardThisTurn)
        return { ok: false, state, error: "already played a dev card this turn" };
    const player = getPlayer(state, userId);
    if (!player || player.devCards[card] <= 0)
        return { ok: false, state, error: "card not available" };
    const next = cloneState(state);
    const nextPlayer = getPlayer(next, userId);
...
function maritimeTrade(state, userId, give, receive) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll dice first" };
    ...
    if (active.id !== userId)
        return { ok: false, state, error: "not your turn" };
    let totalReceive = 0;
    for (const res of Object.keys(receive)) {
        const count = receive[res] || 0;
        if (count < 0 || !Number.isInteger(count))
            return { ok: false, state, error: "invalid receive quantity" };
        totalReceive += count;
    }
    if (totalReceive <= 0)
        return { ok: false, state, error: "must receive at least 1 resource" };
    let totalTradeValue = 0;
    for (const res of Object.keys(give)) {
        const count = give[res] || 0;
        if (count < 0 || !Number.isInteger(count))
            return { ok: false, state, error: "invalid give quantity" };
        if (count === 0)
            continue;
        if ((receive[res] || 0) > 0) {
    ...
    if (totalTradeValue === 0)
        return { ok: false, state, error: "must give at least some resources" };
    ...
    const giveStr = Object.keys(give).filter(r => (give[r] || 0) > 0).map(r => `${give[r]} ${r}`).join(", ");
    const recStr = Object.keys(receive).filter(r => (receive[r] || 0) > 0).map(r => `${receive[r]} ${r}`).join(", ");
    pushLog(next, `${player.username} traded ${giveStr} for ${recStr}.`);
    return { ok: true, state: next };
}
function offerTrade(state, userId, offer, request) {
    if (state.phase !== "main")
        return { ok: false, state, error: "not in main phase" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll dice first" };
...
function acceptTrade(state, userId) {
    ...
    if (!trade)
        return { ok: false, state, error: "no pending trade" };
    if (trade.authorId === userId)
        return { ok: false, state, error: "cannot accept your own trade" };
    const active = getActivePlayer(state);
    if (active.id !== trade.authorId)
        return { ok: false, state, error: "active player changed" };
    const next = cloneState(state);
    const offerer = getPlayer(next, trade.authorId);
    const acceptor = getPlayer(next, userId);
    ...
    for (const resource of Object.keys(trade.offer)) {
        gainResource(acceptor.resources, resource, trade.offer[resource] || 0);
    }
    for (const resource of Object.keys(trade.request)) {
...
function cancelTrade(state, userId) {
    if (!state.pendingTrade)
        return { ok: false, state, error: "no pending trade" };
    if (state.pendingTrade.authorId !== userId)
        return { ok: false, state, error: "only the author can cancel the trade" };
    const next = cloneState(state);
    next.pendingTrade = null;
    const player = getPlayer(next, userId);
...
function endTurn(state, userId) {
    ...
    if (active.id !== userId)
        return { ok: false, state, error: "not your turn" };
    if (state.mustRoll)
        return { ok: false, state, error: "roll before ending turn" };
    if (state.pendingRobber || Object.keys(state.pendingDiscards).length > 0) {
...
function sanitizeStateForUser(state, viewerUserId) {
...
function resetPlayersForLobby(state) {
    for (const player of state.players) {
        player.resources = emptyResources();
        player.roads = [];
        player.settlements = [];
