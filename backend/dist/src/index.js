...
app.post("/api/auth", async (req, res) => {
    ...
    try {
        ...
        const result = { userId: String(user.id), username: user.name };
        usersById.set(result.userId, result.username);
        return res.json(result);
    }
    catch (_err) {
        const fallbackId = `guest_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
        const result = { userId: fallbackId, username: body.username.trim() };
        usersById.set(result.userId, result.username);
        return res.json(result);
    }
});
app.post("/api/admin/reset-lobby", async (_req, res) => {
    try {
        const lobby = createLobbyState("LOBBY");
